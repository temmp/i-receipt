...
/*public class GetImage extends HttpServlet {
 ...
 ----------------------
 query.setFilter("receiptuniqueindex == indexx");
 query.declareParameters("Integer indexx");
 ---------
 List<singer.iReceipt> imgs = (List<singer.iReceipt>) query.execute(user.getEmail(),Integer.parseInt(index));
 -----------
 List<singer.iReceipt> imgs = (List<singer.iReceipt>) query.execute(Integer.parseInt(index));
...
public class GetImage extends HttpServlet {
	public void doGet(HttpServletRequest req, HttpServletResponse resp)
		 ...
		 * query.setFilter("receiptuniqueindex == index");
		 * query.declareParameters("String userid,String index");
		 * List<singer.iReceipt> imgs = (List<singer.iReceipt>)
		 * query.execute(user.getEmail(),index);
		 */

		Query query = pm.newQuery(singer.iReceipt.class);

		query.setFilter("user_id==userid && receiptuniqueindex == index");
		query.declareParameters("String userid,Integer index");

		/*------------------------------
		query.setFilter("receiptuniqueindex == indexx");
		 query.declareParameters("Integer indexx");
		 ----------------------------------*/
		List<singer.iReceipt> imgs;
		if (id != null) {
			imgs = (List<singer.iReceipt>) query.execute(id,
					Integer.parseInt(index));
		} else {
			imgs = (List<singer.iReceipt>) query.execute(user.getEmail(),
					Integer.parseInt(index));
		}
